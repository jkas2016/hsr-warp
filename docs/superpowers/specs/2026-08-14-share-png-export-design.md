...
[내보내기 클릭]
  ...
  1-1. 720px 에서 성립하는 폭 미디어 쿼리 블록을 CSSOM 에서 읽어 <style> 로 컨테이너 안에 주입
...
**1-1번 CSSOM 주입**이 없으면 폭 미디어 쿼리가 컨테이너 폭(720px)이 아니라 **뷰포트 폭**을 보기 때문에
같은 데이터인데도 뷰포트에 따라 다른 그리드가 캡처된다. 방향은 "항상 **720px 기준의 좁은 그리드**"다 —
데스크톱 그리드(4~5열)를 720px 박스에 밀어 넣으면 칸 폭이 화면의 1/3 이하로 좁아져 카드 내용(특히
차트 이미지)이 슬롯을 넘치고 카드 경계에서 잘린다. index.html 의 값을 베끼지 않고, 폭 조건이 720px 에서
성립하는 미디어 블록의 선언을 CSSOM 에서 읽어 `[data-share-box]` 접두사로 재선언한다
(특이성 한 단계 위 → 넓은 뷰포트의 데스크톱 선언을 이기고 실화면에는 매치되지 않음).
좁은 뷰포트에서는 원래 미디어 쿼리도 같은 값을 주므로 390 과 1280 의 결과가 같다.

**고정 폭 720px**이 "화면 크기·스크롤 위치에 결과가 좌우되지 않는다" AC를 담보한다.
AC 는 데스크톱과의 픽셀 동일성이 아니라 **일관성**을 요구한다.
