...
- `expAvg`(이론 기준선)는 **공식 공시의 종합확률(보증 포함)의 역수**다. 추정·모델링이 아니라 공시 원문 값이다.
  - `banners/zzz/<gacha_id>.json` 에 `general_prob_star5` 가 구조화 필드로 들어 있다(예: 독점 `1.600%`, 음의 엔진 `2.000%`). 32개 배너 표본에서 채널별로 값이 완전히 일정했다.
  - 이 산식은 HSR 현행 값을 정확히 재현한다: `1/0.016 = 62.5`(캐릭터), `1/0.0187 ≈ 53.5`(광추). 따라서 두 게임에 같은 근거를 쓴다.

| 채널 | `real_gacha_type` | 4자리 코드 | 기본확률 | 종합확률 | `expAvg` | `cap` | `rateUp` |
|---|---|---|---|---|---|---|---|
| 독점(에이전트) | `2` | 2001·2002·2011·2012 | 0.600% | **1.600%** | **62.5** | 90 | 0.5 |
| 음의 엔진 | `3` | 3001·3002·3011·3012 | 1.000% | **2.000%** | **50.0** | 80 | 0.75 |
| 상시 | `1` | — | 0.600% | 1.600% | 62.5 | 90 | null |
| 본디 | `5` | 5001 | 1.000% | **2.000%** | **50.0** | 80 | null |

4자리 코드의 접두가 `real_gacha_type` 과 대응한다(`2xxx`→`2`, `3xxx`→`3`, `5xxx`→`5`). 특별 채널 `12001`·`13001` 은 각각 독점·음의 엔진과 확률 파라미터가 동일하다.
