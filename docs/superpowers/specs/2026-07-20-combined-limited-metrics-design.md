...
3. 운 지표는 **% 표기를 쓰지 않는다**(2026-07-20 사용자 재결정 — 부호 % 표기가 직관적이지 않음). 기준선 대비 5★당 평균 뽑기 수 **차이(회)**로 표시: 실제>기준이면 "{diff}회 더 많이 뽑았습니다"(불운·빨강), 실제<기준이면 "{diff}회 더 빨리 뽑았습니다"(행운·초록). 개요 카드 큰 숫자 = 차이 회수, 배너별 탭 Mini 도 동일 규칙(`expAvg` 대비). 평균 뽑기 수 카드와의 중복은 이것으로 해소.
