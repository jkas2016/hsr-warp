...
붕괴: 스타레일의 **전언(워프) 기록**을 내 PC로 가져와 **천장·운·픽뚫(50/50)·월별 통계**를 보여주는 작은 프로그램입니다. 설치 마법사로 설치하고 실행하면 브라우저에 대시보드가 자동으로 열립니다. 앱과 가이드는 **한국어·영어·중국어(간체)·일본어**를 지원합니다.
...
- 신규 패치 출시 시 `web/schedule.json`의 `schedule` 배열에 새 페이즈 `{s,e,c,l}`를 추가하고 최상위 `version`을 +1 하세요(c=캐릭터·l=광추 픽업 item_id; 픽업=Mantan21/HSR-Warp-Simulator, item_id=StarRailRes). `main`에 push 하면 사용자 앱이 시작 시 자동으로 받아 반영합니다(릴리스 불필요). `npm run schedule:status` 로 현재 데이터가 어디까지 커버하는지 바로 확인할 수 있습니다. 누락 시 그 시점 5★가 '미확인'으로 표시됩니다.
