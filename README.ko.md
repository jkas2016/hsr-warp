...
> ⚠️ **가져올 수 있는 기간** — 게임 서버가 돌려주는 범위까지만 조회됩니다. **젠레스 존 제로는 최근 약 6개월치만** 반환하므로 그보다 오래된 변조 기록은 처음 조회하는 시점에 이미 가져올 수 없습니다. 붕괴: 스타레일은 실측상 6개월보다 오래된 전언 기록도 함께 반환됐습니다(호요버스가 언제든 바꿀 수 있는 서버 쪽 정책입니다). 한 번 가져온 기록은 이 프로그램이 계속 보존하니, 일찍 시작할수록 더 긴 기록이 남습니다.
